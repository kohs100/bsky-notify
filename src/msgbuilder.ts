...
export default class InteractiveMessage {
  ...
  async _like() {
    ...
    const { uri: uri_like } = await singleton.client.agent.like(
      this.uri,
      this.cid
    );
  ...
  async _repost() {
    ...
    const { uri: uri_repost } = await singleton.client.agent.repost(
      this.uri,
      this.cid
    );
  ...
  async send() {
    ...
    collector.on("end", async (collected, reason) => {
      ...
      try {
        ...
        singleton.catch(
          e,
          `MsgComponentCollector event failed: end: ${reason}.`
        );
      }
    });
    ...
    collector.on("collect", async i => {
      ...
      if (tokens[1] == "bsky") {
        try {
          if (tokens[2] == "like") {
            if (this.liked) await this._unlike();
            else await this._like();
          } else if (tokens[2] == "repost") {
            if (this.reposted) await this._unrepost();
            else await this._repost();
          } else {
            timedLog(i);
